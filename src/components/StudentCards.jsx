...
export default function StudentCards() {
  ...
  const students = useMemo(
    () => classes
      ...
      .filter(({ student }) => student.name.toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())),
    [classes, classId, query],
  )
  ...
  return (
    <div>
      ...
      <p className="mb-4 max-w-2xl text-sm text-ink/60">Your private workspace for student history, contacts, support plans, and follow-up. Open a card for the full details.</p>
          ...
          {students.map(({ cls, student }) => {
            ...
            return (
              <button
                ...
                className="sticker flex min-h-40 flex-col rounded-3xl bg-white p-4 text-left transition-all hover:-translate-y-0.5 hover:scale-[1.02] cursor-pointer"
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  {plans[student.id]?.type && <span className="rounded-full bg-violet-100 px-2 py-1 text-xs font-bold text-violet-900">Support plan</span>}
                </div>
